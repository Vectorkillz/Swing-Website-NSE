...
def regime_effects(regime: Regime, cfg: ScanConfig) -> tuple[bool, bool]:
    """(longs_allowed, shorts_allowed). One explicit table."""
    table = {
        Regime.BULL: (True, True),
        Regime.BULL_HIGH_VIX: (True, cfg.allow_shorts_in_bull_high_vix),
        Regime.NEUTRAL: (True, True),
        Regime.BEAR: (False, True),
        Regime.UNKNOWN: (False, False),
    }
    return table[regime]


def _unknown(reasons: list[str], **kw) -> RegimeResult:
    return RegimeResult(regime=Regime.UNKNOWN, longs_allowed=False, shorts_allowed=False, reasons=tuple(reasons), **kw)


def classify_regime(nifty: pd.DataFrame | None, vix_close: float | None, smallcap: pd.DataFrame | None, cfg: ScanConfig) -> RegimeResult:
    reasons: list[str] = []
    ...
    above_fast, above_slow = cur > e_fast, cur > e_slow
    ...
    return RegimeResult(
        regime=regime, longs_allowed=longs, shorts_allowed=shorts, nifty_close=cur, nifty_ema_fast=e_fast, nifty_ema_slow=e_slow,
        roc_18m=roc_18m, roc_bars_used=lookback, vix=float(vix_close), smallcap_confirms=sc_conf, smallcap_close=sc_close,
        smallcap_ema_fast=sc_fast, smallcap_ema_slow=sc_slow, reasons=tuple(reasons),
    )
